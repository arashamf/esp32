...
void send_hor_blocks(spi_device_handle_t spi, int ypos, uint16_t *data)
{
  ...
  for (x=0; x<6; x++) {
    ...
    if ((x&1)==0)
    {
        trans[x].length=8; //Even transfers are commands
        trans[x].user=(void*)0;
    }
    else
    {

        trans[x].length=8*4; //Odd transfers are data
        trans[x].user=(void*)1;
    }
...
void send_block_finish(spi_device_handle_t spi)
{
    ...
    for (int x=0; x<6; x++) //Wait for all 6 transactions to be done and get back the results
    {
        ret=spi_device_get_trans_result(spi, &rtrans, portMAX_DELAY);
        assert(ret==ESP_OK);  //We could inspect rtrans now if we received any info back. The LCD is treated as write-only, though.
    }
}
...
void LCD_FillRect(spi_device_handle_t spi, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
  ...
  blck=heap_caps_malloc(size_max*sizeof(uint16_t), MALLOC_CAP_DMA);  //swap bytes;
  color = color<<8 | color>>8;
...
void LCD_SetAddrWindow(spi_device_handle_t spi, uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
  ...
  lcd_cmd(spi, 0x2A); // column address set (CASET)
  {
    uint8_t data[] = { (x0 >> 8) & 0xFF, x0 & 0xFF, (x1 >> 8) & 0xFF, x1 & 0xFF };
    LCD_WriteData(spi, data, sizeof(data));
  }
  lcd_cmd(spi, 0x2B); // row address set (RASET)
  {
    uint8_t data[] = { (y0 >> 8) & 0xFF, y0 & 0xFF, (y1 >> 8) & 0xFF, y1 & 0xFF };
    LCD_WriteData(spi, data, sizeof(data));
  }

  lcd_cmd(spi, 0x2C); // write to RAM (RAMWR)
}
...
void LCD_DrawChar(spi_device_handle_t spi, uint16_t x, uint16_t y, uint8_t c)
{
  ...
  uint8_t *pchar;
  uint32_t line=0;
  uint16_t height = lcdprop.pFont->Height;
  uint16_t width  = lcdprop.pFont->Width;
...
void LCD_Refresh(spi_device_handle_t spi)
{
	uint8_t data[2];
	uint16_t x, y;
//	LCD_SetAddrWindow(spi, 0, 0, LCD_WIDTH-1, LCD_HEIGHT-1);

//	lcd_cmd(spi, 0x2C);  // write to RAM
	for (x=0; x < (LCD_WIDTH*LCD_HEIGHT); x++)
	{
